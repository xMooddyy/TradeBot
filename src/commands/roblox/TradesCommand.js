...
const getTrade = async (tradeId) => {
    const item = await Roblox_1.default.apis.tradesAPI.getTrade({ tradeId });
    if (!item)
        return null;
    return item;
};
class TradesCommand extends a_djs_handler_1.BaseCommand {
    ...
    async run(client, message, args) {
        ...
        const rawTrades = await Roblox_1.default.apis.tradesAPI.getTradesByStatusType({ tradeStatusType: 'Inbound', limit: 100 }).catch(() => null);
        if (!rawTrades || !rawTrades.data.length)
            return msg.edit('There are no trades.');
        const trades = [];
        const promises = await Promise.all(rawTrades.data.map((trade) => getTrade(trade.id)));
        if (args[0]) {
            const user = await Roblox_1.default.getUserIdFromUsername(args[0]).catch(() => null);
            if (!user)
